...
class OLMoBlock(nn.Module):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        if config.attention_layer_norm:
            ...
            if config.use_olmo2_original_qk_norm:
                q_norm_dim, k_norm_dim = config.d_model, (config.d_model // config.n_heads) * config.effective_n_kv_heads
    ...
    def attention(
        ...
        if self.config.use_olmo2_original_qk_norm:
        ...
        if not self.config.use_olmo2_original_qk_norm:
